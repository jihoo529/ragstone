...
**Getting started: Hello enclave ........................................................................................................ 8**
...
[For more information about the optional fields in the attestation document, see the Nitro Enclaves](https://github.com/aws/aws-nitro-enclaves-nsm-api/blob/main/docs/attestation_process.md)
...
that contains the signed attestation document. The signed attestation document is a CBORencoded, COSE-signed (using the COSE_Sign1 signature structure) object. The overall validation
...
[certificate for the commercial AWS partitions. The root certificate can be downloaded from https://](https://aws-nitro-enclaves.amazonaws.com/AWS_NitroEnclaves_Root-G1.zip)
[aws-nitro-enclaves.amazonaws.com/AWS_NitroEnclaves_Root-G1.zip, and it can be verified using](https://aws-nitro-enclaves.amazonaws.com/AWS_NitroEnclaves_Root-G1.zip)
...
```
[Keep this ordering in mind, as some existing tools, such as Java’s CertPath from Java PKI API](https://docs.oracle.com/javase/8/docs/technotes/guides/security/certpath/CertPathProgGuide.html)
[Programmer’s Guide, might require them to be ordered differently.](https://docs.oracle.com/javase/8/docs/technotes/guides/security/certpath/CertPathProgGuide.html)
...
[Currently, ACM for Nitro Enclaves works with NGINX servers and Apache HTTP servers running on](https://www.nginx.com/)
...
[you use and pay for ACM Private CA.](https://aws.amazon.com/certificate-manager/pricing/)
...
information about creating a certificate, see the following resources in the AWS Certificate Manager
_User Guide._
...
1. [Open the ACM for Nitro Enclaves page in the AWS Marketplace.](https://aws.amazon.com/marketplace/pp/B08S7NZFNF)
...
```
 $ aws ec2 run-instances --image-id ami_id --count 1 --instance type supported_instance_type --key-name your_key_pair --enclave-options
 'Enabled=true'
...
```
[Use the create-role command to create a role named acm-role, and specify the path to the JSON](https://docs.aws.amazon.com/cli/latest/reference/iam/create-role.html)
policy file.
...
[Attach the IAM role that you created in the previous step to the ACM certificate. To do this, use the](https://docs.aws.amazon.com/cli/latest/reference/ec2/associate-enclave-certificate-iam-role.html)
[associate-enclave-certificate-iam-role command, and specify the ARN of the role to attach, and](https://docs.aws.amazon.com/cli/latest/reference/ec2/associate-enclave-certificate-iam-role.html)
...
```
 ...
 arn:aws:acm:us-east-1:123456789012:certificate/d4c3b2a1-e5d0-4d51-95d9-1927fEXAMPLE - role-arn arn:aws:iam::123456789012:role/acm-role
...
```
[Use the put-role-policy command to add the additional policies to the acm-role role, and specify](https://docs.aws.amazon.com/cli/latest/reference/iam/put-role-policy.html)
...
[Create a new instance profile named acm-instance-profile using the create-instance-profile](https://docs.aws.amazon.com/cli/latest/reference/iam/create-instance-profile.html)
command.
```
...
[Use the add-role-to-instance-profile command.](https://docs.aws.amazon.com/cli/latest/reference/iam/add-role-to-instance-profile.html)
```
 $ aws iam add-role-to-instance-profile --instance-profile-name acm-instance-profile - role-name acm-role

```
[Associate the instance profile with the instance that you launched previously. Use the associate-](https://docs.aws.amazon.com/cli/latest/reference/iam/associate-iam-instance-profile.html)
[iam-instance-profile command and specify the instance profile to attach and the instance to attach](https://docs.aws.amazon.com/cli/latest/reference/iam/associate-iam-instance-profile.html)
it to.
```
...
[information about generating the certificate chain for your certificate, see Exporting a](https://docs.aws.amazon.com/acm/latest/userguide/export-private.html#export-console)
[Private Certificate in the AWS Certificate Manager User Guide.](https://docs.aws.amazon.com/acm/latest/userguide/export-private.html#export-console)
```
...
[If you renew the ACM certificate by running acm renew-certificate, the ACM for Nitro](https://docs.aws.amazon.com/cli/latest/reference/acm/renew-certificate.html)
...
[domains and about different use cases, refer to the NGINX documentation or Apache HTTP](https://nginx.org/en/docs/)
[server documentation.](https://httpd.apache.org/docs/)
...
[Compliance Programs. To learn about the compliance programs that apply to Amazon EC2, see](https://aws.amazon.com/compliance/programs/)
...
[• Amazon EC2 and interface VPC endpoints](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/interface-vpc-endpoints.html)
...
[• Identity and access management for Amazon EC2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/security-iam.html)
...
[• Update management in Amazon EC2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/update-management.html)

[• Compliance validation for Amazon EC2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/compliance-validation.html)
...
[• kms-decrypt (Decrypt)](https://docs.aws.amazon.com/kms/latest/APIReference/API_Decrypt.html)

[• generate-data-key (GenerateDataKey)](https://docs.aws.amazon.com/kms/latest/APIReference/API_GenerateDataKey.html)

[• generate-random (GenerateRandom)](https://docs.aws.amazon.com/kms/latest/APIReference/API_GenerateRandom.html)
...
[Nitro CLI on different Linux distributions, see the Nitro CLI github repository.](https://github.com/aws/aws-nitro-enclaves-cli)
...
[complete the Distributor prerequisites.](https://docs.aws.amazon.com/systems-manager/latest/userguide/distributor-prerequisites.html)
...
[Manager Distributor. For more information see Uninstall a package in the AWS Systems Manager](https://docs.aws.amazon.com/systems-manager/latest/userguide/distributor-working-with-packages-uninstall.html)
_User Guide._
...
```
 ...
 dockerfile_directory --output-file enclave_image_filename --private-key key.pem - signing-certificate certificate.pem
...
```
 ...
 | --cpu-ids list_of_vcpu_ids] --memory amount_of_memory_in_MiB --eif path path_to_enclave_image_file [--enclave-cid cid_number] [--debug-mode]
...
[on the instance. For more information about vCPUs, see Optimize CPU options in the](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/instance-optimize-cpu.html)
_Amazon EC2 User Guide._
...
_vsock socket. Only CIDs of 4 and higher can be specified. If you omit this option, a random CID is_
...
```
 {
   "PCR8":
 "example39de75e8ed2939e95examplea96f2c79eaf5d5ac3bacf2cb76c75a31f9examplef55b29f0acd256b8examp
 }
...
[Multiple enclaves per instance](https://docs.aws.amazon.com/enclaves/latest/user/multiple-enclaves.html) You can run up to four Nitro
Enclaves on a single parent
Amazon EC2 instance.

[Amazon EKS support](https://docs.aws.amazon.com/enclaves/latest/user/kubernetes.html) You can use Amazon Elastic
Kubernetes Service to
orchestrate, scale, and
deploy Nitro Enclaves from a
Kubernetes pod.

[AWS Graviton support](https://docs.aws.amazon.com/enclaves/latest/user/nitro-enclave.html#nitro-enclave-reqs) Nitro Enclaves now supports
AWS Graviton-based Amazon
EC2 instance types, except A1,
G5g, Im4gn, Is4gen, and T4g.

[ACM for Nitro Enclaves](https://docs.aws.amazon.com/enclaves/latest/user/nitro-enclave-refapp.html) ACM for Nitro Enclaves now
...
[Nitro Enclaves 1.2.0 for](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications-windows.html#release-notes)
[Windows](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications-windows.html#release-notes)

[Amazon SNS topic for](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications-windows.html#sns-topic)
[Nitro Enclaves for Windows](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications-windows.html#sns-topic)
[updates](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications-windows.html#sns-topic)

[Nitro Enclaves 1.1.0 for](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications-windows.html#release-notes)
[Windows](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications-windows.html#release-notes)
...
[Nitro Enclaves on Windows](https://docs.aws.amazon.com/enclaves/latest/user/developing-applications-windows.html) Nitro Enclaves supports
...
[Initial release](https://docs.aws.amazon.com/enclaves/latest/user/nitro-enclave.html) Initial release of AWS Nitro
